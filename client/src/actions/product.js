...
export function createProduct(prod) {
  return (dispatch, getState) => {
    ...
    return Axios.post("http://localhost:4000/products", prodEnviar, config)
      .then((res) => res.data)
      .then((res) => dispatch({ type: CREATE_PRODUCT, payload: res }));
  };
}
...
export function updateProduct(id, prod) {
  return (dispatch, getState) => {
    ...
    return Axios.put(
      ...
      prodEnviar,
      config
    ).then((res) => dispatch({ type: UPDATE_PRODUCT, payload: res }));
  };
}
...
export function deleteProduct(id) {
  return (dispatch, getState) => {
    ...
    return Axios.delete(
      "http://localhost:4000/products/" + id,
      config
    ).then((res) => dispatch({ type: DELETE_PRODUCT, payload: res }));
  };
}
...
export function addCategoryToProduct(cat, id) {
  return (dispatch, getState) => {
    ...
    return Axios.post(
      "http://localhost:4000/products/" + id + "/category/" + cat,
      config
    ).then((res) => dispatch({ type: ADD_CATEGORY_TO_PRODUCT, payload: res }));
  };
}
